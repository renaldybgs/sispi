...
class Controller_EngineerListIpkc extends Controller
{
    public function openPage(){         //buka halaman Engineer - Project Own Going (Own Project)
        ...
        $cas = DB::table('cas')->select('nama_issuer')->orderBy('nama_issuer', 'ASC')->get();
        $pstat = DB::table('projects_stats')->select('id' , 'nama_pstat')->orderBy('id', 'ASC')->get();
        
        return view('Pages.Engineer.View_EngineerListIpkc', compact('userLevel', 'pic', 'cas', 'pstat'));
    }
    ...
    public function getPBN($id){
        return DB::table('ipkcs')
        ->select(DB::raw('ipkcs.id, DATE(ipkcs.waktu_assign_project) as waktu, ipkcs.no_ipkc, ipkcs.bin, ipkcs.notes_project, ipkcs.jenis_ipkc'))
        ->where('ipkcs.id', '=', $id)
        ->orderBy('waktu')
        ->first();
    }
